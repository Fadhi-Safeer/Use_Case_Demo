...
# ── Module-level session state (one session at a time) ────────────────────────

_inference_queue:    asyncio.Queue = None
_producer_task:      asyncio.Task  = None
_consumer_task:      asyncio.Task  = None
_current_session_id: str           = ""
_current_use_case:   str           = ""
...
_last_warned_tier:    int  = 0
_stuck_warning_fired: bool = False

# Per-session counters
_session_start_ts: float = 0.0
_total_captured:   int   = 0
_total_inferred:   int   = 0


def _reset_session_stats():
    global _last_warned_tier, _stuck_warning_fired
    global _session_start_ts, _total_captured, _total_inferred
    _last_warned_tier             = 0
    _stuck_warning_fired          = False
...
def _check_queue_warnings(queue: asyncio.Queue):
    ...
    # Rising edge only
    if fired_tier > _last_warned_tier:
        log.log(fired_level, f"[queue] depth={depth}: {fired_msg}")
        _last_warned_tier = fired_tier
...
async def _consumer(queue: asyncio.Queue):
    global _total_inferred, _stuck_warning_fired
...
async def _shutdown_session(session_id: str):
    ...
    duration = round(time.monotonic() - _session_start_ts, 1)
    summary  = {
        "session_id":               session_id,
        "total_captured":           _total_captured,
        "total_inferred":           _total_inferred,
...
async def start_live(data: dict):
    global _inference_queue, _producer_task, _consumer_task
    global _current_session_id, _current_use_case
    ...
    _producer_task   = asyncio.create_task(
        _producer(_inference_queue, session_id, use_case, prompt, system_prompt, interval)
    )
    _consumer_task   = asyncio.create_task(
        _consumer(_inference_queue)
    )
    state.live_mode_task = _producer_task
...
async def stop_live():
    ...
    asyncio.create_task(_shutdown_session(_current_session_id))
